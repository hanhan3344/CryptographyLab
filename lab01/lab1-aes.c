...
{//请补充代码
    int K[4];
    for (int i = 0 ; i < 4 ; i++){
        splitIntToArray(w[round * 4 + i], K);
        for(int j = 0 ; j < 4 ; j++){
            array[j][i] = array[j][i] ^ K[j];
        }
    }
}
...
{//请补充代码
    int num_to_array[4] = {0};
    int res;

    splitIntToArray(num, num_to_array); // num转为4位
    leftLoop4int(num_to_array, 1); // 循环左移一位
    for(int i = 0 ; i < 4 ; i++){
        num_to_array[i] = getNumFromSBox(num_to_array[i]);
    }
    res = mergeArrayToInt(num_to_array);
